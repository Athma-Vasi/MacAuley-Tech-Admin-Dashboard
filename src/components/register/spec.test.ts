...
async function handleRegisterButtonClickTestMock(
  ...
  const imagePath = "../../assets/macauley-tech-full.jpeg"; // path to your test image
  const buffer = readFileSync(imagePath);
  const file = new File([buffer], basename(imagePath), { type: "image/jpeg" });

  const formData = new FormData();
  formData.append("schema", JSON.stringify({ schema }));
  formData.append("macauley-testing-image", file, "TEST.jpg");
  ...
  try {
    ...
    const registerResultUnwrapped =
      handleRegisterButtonClickResult.safeUnwrap().data;
    if (registerResultUnwrapped === undefined) {
      return createSafeBoxResult({
        message: "No data returned from server",
      });
    }
    ...
    const { accessToken, userDocument } = loginUnwrapped[0];
